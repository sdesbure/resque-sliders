$(document).ready(function() {
  ...
  $(".controls").find("span").click(function() {
    ...
    var span = $(this);
    $.post('/resque/sliders/' + host, signal, function(data) {
      switch(data.signal) {
        case 'reload':
  ...
  $('.new_form').submit(function(e) {
    ...
    $.post('/resque/sliders/' + host, { quantity: 1, queue: queue }, function(data) {
      // reload window
  ...
  $(".slider").each(function() {
    ...
    slidy.empty().slider({
      ...
      change: function( event, ui ) {
        $.post('/resque/sliders/' + host, { quantity: ui.value, queue: queue }, function(data) {
        });
      },
    });
  });
